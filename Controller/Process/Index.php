...
class Index extends Action implements HttpPostActionInterface, CsrfAwareActionInterface
{
    ...
    private function processCallback($data)
    {
        try {
            ...
            if (!$this->verifySignature($data, $objectManager)) {
    ...
    private function verifySignature($data, $objectManager)
    {
        if (isset($data['card'])) {
            $card = $data['card'];
        ...
        return $this->getSignature($data['email'], $data['order'], $card, $objectManager) === $data['sign'];
    }

    private function getSignature($email, $order, $card, $objectManager)
    {
        $pass = $objectManager->get(\Magento\Framework\App\Config\ScopeConfigInterface::class)
            ->getValue('payment/platon/pass');
        $email = strrev($email);
